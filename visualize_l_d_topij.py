...
def main(args):
    ...
    for col, model_name in enumerate(models):
        ...
        for idx_nk, n_k in enumerate(n_k_list):
            ...
            try:
                ...
                line0, = ax[0][col].plot(sampled_indices, mean_score_0[sampled_indices], 
                                         label=f"top_k={n_k}", color=colors[idx_nk],
                                         linestyle=linestyles[idx_nk], marker=markers[idx_nk],
                                         linewidth=2, markersize=6)
                line1, = ax[1][col].plot(sampled_indices, mean_score_1[sampled_indices], 
                                         label=f"top_k={n_k}", color=colors[idx_nk],
                                         linestyle=linestyles[idx_nk], marker=markers[idx_nk],
                                         linewidth=2, markersize=6)
    ...
    # Legend ngoài toàn bộ figure
    fig.legend(all_lines, all_labels, loc='center left', bbox_to_anchor=(1.01, 0.5),
               frameon=True, fancybox=True, shadow=True, facecolor='white', framealpha=0.5)
