...
def main(args):
    ...
    for folder_name in tqdm(os.listdir(args.extracted_path)):
        ...
        pred_answers = meta_data.get("llm_answer", meta_data.get("lvlm_answers", ""))

        system_prompt, user_prompt = get_prompt_compare_answer(
            gt_answer=gt_answer,
            model_answer=pred_answers[args.n_k - 1],
            question=question
        )
        ...
        score_file_path = os.path.join(args.extracted_path, folder_name, f"score_{args.n_k}.txt")
